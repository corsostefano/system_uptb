...
if ($first_name_post == '' || $last_name_post == '' || $rol_id == '' || $email == '' || $identification_type == '' || $cedula == '' || $sex == '' || $marital_status == '' || $university_campus == '' || $education == '' || $profession == '' || $address_user == '' || $state_of_residence == '' || $municipality == '' || $rol_category == '' || $specialty == '' || $seniority == '' || $phone == '' || $mobile == '' || $emergency_contact == '')  {
    session_start();
    $_SESSION['message'] = "Error en Registro, por favor complete todos los campos";
    $_SESSION['icon'] = "error";
    header('Location: ' . APP_URL . "/admin/teachers/create.php");
    exit(); 
} else {
